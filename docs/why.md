...
`go-ruby-msgpack/msgpack` reimplements Ruby's `msgpack` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-msgpack/msgpack` directly, with no Ruby runtime;
